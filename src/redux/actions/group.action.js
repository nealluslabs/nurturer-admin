...
export const createGroup =
  (groupData, user, file, navigate, setLoading, url) => async (dispatch) => {
    var options = {
      weekday: "long",
      year: "numeric",
      month: "long",
...
export const updateTriggerDaysForAllContacts =
  (newTriggerDays) => async (dispatch) => {
    try {
...
export const updateSettingsForAdminSettings =
  (updateObject) => async (dispatch) => {
...
export const updateAllContacts = () => async (dispatch) => {
  try {
    const oldPhotoUrl =
      "https://nurturer.s3.eu-west-3.amazonaws.com/no-pic.png";
    const newPhotoUrl =
...
export const simulateCronJob = () => async (dispatch) => {
  try {
    ...
    const generateAiMessage = async (
      ...
      Interests,
      previousMessage,
      adminSettingsTriggerDays,
    ) => {
      //do not delete the other argumens they are being called by the query in the eval
      ...
      try {
        ...
        console.log(
          "OUR RESPONSE FROM OUR BACKEND, WHICH CALLS CHAT GPT-->",
          jobResponse.data,
        );

        const fullJobDetailsResponse =
          /*JSON.parse(jobResponse.data)*/ jobResponse.data;
        ...
        if (error.response) {
          ...
          console.error(
            "⚠️ No response received from OpenAI API:",
            error.request,
          );
        } else {
    ...
    for (const doc of snapshot.docs) {
      ...
      const birthdayParts =
        data && data.birthday
          ? data.birthday.split("/")
          : "1/1/1980".split("/"); // Split the date string (DD/MM/YYYY)
      ...
      const currentBirthdaySendDateNumInDays = Math.floor(
        timeDifferenceBirthday / (1000 * 60 * 60 * 24),
      );
      ...
      console.log(
        "WHAT IS ADMIN SETTINGS TRIGGER DAYS --->",
      ...
      if (
        ...
        let updatedBirthdaySendDate =
          data.birthdaySendDate && data.birthdaySendDate; //dont need this anymore - i am using the contacts birthday
        let updatedHolidaySendDate =
          data.holidaySendDate && data.holidaySendDate; //dont need this anymore - i am using hardcoded days of july 4, christmans, new years
        ...
        if (
          currentSendDateNum ===
            (adminSettings && Number(adminSettings.triggerDays)) &&
          data &&
          data.touchesAlert === true
        ) {
          console.log(
            "GENERATE AI MESSAGE IS ABOUT TO TRIGGER FOR--->",
            data.name,
          );
          aiGeneratedMessage = await generateAiMessage(
            ...
            userDoc
              .data()
              .queryMsg?.find((item) => item.messageType === "Email"),
            adminSettings && Number(adminSettings.triggerDays),
            // userDoc.data(),
            // data
          );
        ...
        if (
          currentBirthdaySendDateNumInDays ===
            (adminSettings && Number(adminSettings.triggerDays)) &&
          data &&
          data.eventsAlert === true
        ) {
          aiGeneratedMessage = await generateAiMessage(
            "Birthday",
            ...
            userDoc
              .data()
              .queryMsg?.find((item) => item.messageType === "Birthday"),
            adminSettings && Number(adminSettings.triggerDays),
            // userDoc.data(),
            // data
          );
        ...
        if (
          ...
          aiGeneratedMessage = await generateAiMessage(
            ...
            userDoc
              .data()
              .queryMsg?.find((item) => item.messageType === "Holiday"),
            adminSettings && Number(adminSettings.triggerDays),
            // userDoc.data(),
            // data
          );
        ...
        if (
          (currentSendDateNum === thanksgivingDays) ===
            (adminSettings && Number(adminSettings.triggerDays)) &&
          data &&
          data.eventsAlert === true
        ) {
        ...
        console.log(
          "RAW MESSAGE THAT WAS JUST GENERATED BY AI -->",
          aiGeneratedMessage,
        );
        ...
        updatedSendDate =
          currentSendDateNum > 1
            ? String(currentSendDateNum - 1)
            : String(data.frequencyInDays);

        const updatedMessage = {
          firstParagraph: aiGeneratedMessage
            ? aiGeneratedMessage.firstParagraph
            : "",
          secondParagraph: aiGeneratedMessage
            ? aiGeneratedMessage.secondParagraph
            : "",
          thirdParagraph: aiGeneratedMessage
            ? aiGeneratedMessage.thirdParagraph
            : "",
          bulletPoints: aiGeneratedMessage
            ? aiGeneratedMessage.bulletPoints
            : "",
        ...
        if (currentSendDateNum === 1) {
          ...
          if (data.touchesAlert !== null && data.touchesAlert === true) {
                            ...
                            ? data.messageQueue[
                                ...
                                .map(
                                  ...
                                  <li>
                                    <strong>${
                                      bp.bulletPointBold || ""
                                    }</strong> — ${bp.bulletPointRest || ""} 
                                    <a href="${
                                      bp.link || "#"
                                    }" target="_blank">${bp.link || ""}</a>
                                  </li>`,
                                )
                                .join("")
                            : ""
            ...
            // Get the last (most recent) email
            const mostRecentEmail = emailMessages
        ...
        const isTodayHoliday =
          christmasDays === 0 || newYearsDays === 0 || independenceDays === 0;
        let whichHoliday = "";
        ...
        const isHolidayAdminSendDate =
          (christmasDays === adminSettings &&
            Number(adminSettings.triggerDays)) ||
          (newYearsDays === adminSettings &&
            Number(adminSettings.triggerDays)) ||
          (independenceDays === adminSettings &&
            Number(adminSettings.triggerDays)) ||
          (laborDays === adminSettings && Number(adminSettings.triggerDays)) ||
          (memorialDays === adminSettings &&
            Number(adminSettings.triggerDays)) ||
          (thanksgivingDays === adminSettings &&
            Number(adminSettings.triggerDays));
        ...
        const isSendDateAdminSendDate =
          Number(data.sendDate) === Number(adminSettings.triggerDays);

        contactsLog.push({
          contactName: data.name,
          contactEmail: data.email,
          contactId: data.uid,
          emailSubject: aiGeneratedMessage
            ? aiGeneratedMessage.subject
          ...
          generatedMessage: {
            subject: aiGeneratedMessage ? aiGeneratedMessage.subject : " ",
            firstParagragph: aiGeneratedMessage
              ? aiGeneratedMessage.firstParagraph
              : "",
            secondParagragph: aiGeneratedMessage
              ? aiGeneratedMessage.firstParagraph
              : "",
          },
    ...
    if (writeCount > 0) {
      ...
      committedBatches++;
      console.log(
        `Committed final batch #${committedBatches} with ${writeCount} writes`,
      );
    }
            ...
            Body: {
              Html: {
                ...
                ${
                  generatedContacts &&
                  generatedContacts
                    .map((contact) => `${contact.name} - ${contact.event}`)
                    .join(", ")
                }
    ...
    return notifySuccessFxn(
      "Contacts Updated Successfully, emails have been sent out!",
    );
  } catch (error) {
    console.error(error);
...
export const uploadUserSettings =
  (groupData = 0, file = 0, user = 0) =>
  async (dispatch) => {
              ...
              if (groupData.newPassword) {
                ...
                fb.auth()
                  ...
                  .then((userCredential) => {
                    ...
                    user
                      .updatePassword(groupData.newPassword)
                      .then(() => {
                        // Update successful.
                  ...
                  .catch(() => {
                    notifyErrorFxn(
                      "Please try updating your password again...",
                    );
                  });
              }
    ...
    if (file.length === 0 && groupData.newPassword) {
      ...
      fb.auth()
        .signInWithEmailAndPassword(groupData.email, groupData.password)
        .then((userCredential) => {
          // Signed in
          const user = fb.auth().currentUser;
...
export const fetchMyGroups = (coolers) => async (dispatch) => {
  ...
  if (coolers.length) {
    const chunkSize = 10;
    const chunks = coolers.reduce(
      (acc, _, i) =>
        i % chunkSize ? acc : [...acc, coolers.slice(i, i + chunkSize)],
      [],
    );
  ...
  db.collection("sections")
    ...
    .then((snapshot) => {
      ...
      if (allSectionVids.length > 0) {
        //dispatch(isItLoading(false));
        console.log(
          "ALL sections FROM DATABASE(FOR THIS CATEGORY):",
          sortedSectionVids,
        );
...
export const fetchSubjectsInPackDetails =
  (subjectsInPack) => async (dispatch) => {
    //dispatch(isItLoading(true));
    db.collection("sections")
      ...
      .then((snapshot) => {
        ...
        if (allSectionVids.length > 0) {
          //dispatch(isItLoading(false));
          console.log(
            "ALL sections FROM DATABASE(FOR THIS PACK ARE--->):",
            sortedSectionVids,
          );
          dispatch(saveCategoryVideos(sortedSectionVids));
        } else {
...
export const fetchSubjectChapters = (chosenSection) => async (dispatch) => {
  db.collection("pastExams")
    ...
    .then((snapshot) => {
      ...
      if (pastExamsArray.length) {
        console.log(
          `pastExams for The subject ${chosenSection} are:`,
          pastExamsArray,
        );
  ...
  db.collection("chapters")
    ...
    .get()
    .then((snapshot) => {
      const allSectionChapters = snapshot.docs.map((doc) => ({
        ...doc.data(),
      }));
      const sortFunction = (array) => {
        if (array.length) {
...
export const fetchChapterSessions = (chosenChapter) => async (dispatch) => {
  ...
  db.collection("boneCourses")
    ...
    .get()
    .then((snapshot) => {
      const allChapterSessions = snapshot.docs.map((doc) => ({
        ...doc.data(),
      }));
      const sortFunction = (array) => {
        if (array.length) {
      ...
      if (allChapterSessions.length > 0) {
        //dispatch(isItLoading(false));
        console.log(
          "ALL sessions FROM DATABASE(FOR THIS CHAPTER):",
          sortedChapterSessions,
        );
  ...
  db.collection("quizzes")
    ...
    .then((snapshot) => {
      ...
      if (allChapterQuizzes.length > 0) {
        //dispatch(isItLoading(false));
        console.log(
          "ALL quizzes FROM DATABASE(FOR THIS CHAPTER):",
          sortedChapterQuizzes,
        );
...
export const deleteChapter = (chosenChapter, navigate) => async (dispatch) => {
  ...
  db.collection("quizzes")
    ...
    .then(async (snapshot) => {
      ...
      if (allChapterQuizzes.length > 0) {
        notifyErrorFxn(
          "Could not delete Chapter, please make sure to delete all quizzes within that chapter first",
        );
        dispatch(isItLoading(false));
        return;
      } else {
        db.collection("boneCourses")
          ...
          .then(async (snapshot) => {
            ...
            if (allChapterSessions.length > 0) {
              notifyErrorFxn(
                "Could not delete Chapter, please make sure to delete all lessons within that chapter first",
              );
              dispatch(isItLoading(false));
              return;
            } else {
              let sectionId;
              let itemToBeDeleted = db
                .collection("chapters")
                .doc(chosenChapter);

              await itemToBeDeleted
                .get()
                .then((doc) => {
                  if (doc.exists) {
                    ...
                    notifyErrorFxn(
                      "Problem Deleting the Chapter, please try again",
                    );
                    dispatch(isItLoading(false));
                    return;
                  }
...
export const fetchSinglePack = (packId) => async (dispatch) => {
  db.collection("packs")
    ...
    .catch((error) => {
      console.log(
        "Error fetching a particular subject from sections collection:",
        error,
      );
    });
};
...
export const fetchChapterInfo = (uid) => async (dispatch) => {
  db.collection("chapters")
    ...
    .catch((error) => {
      console.log(
        "Error fetching a particular chapter from chapters collection:",
        error,
      );
    });
};
...
export const fetchLessonInfo = (uid) => async (dispatch) => {
  db.collection("boneCourses")
    ...
    .catch((error) => {
      console.log(
        "Error fetching a particular lesson from boneCourses collection:",
        error,
      );
    });
};
...
export const fetchPastExamInfo = (uid) => async (dispatch) => {
  db.collection("pastExams")
    ...
    .catch((error) => {
      console.log(
        "Error fetching a particular past exam from past exams collection:",
        error,
      );
    });
};
...
export const fetchQuizInfo = (uid) => async (dispatch) => {
  db.collection("quizzes")
    ...
    .catch((error) => {
      console.log(
        "Error fetching a particular quiz from quizzes collection:",
        error,
      );
    });
};
...
export const fetchTeacherInfo = (uid) => async (dispatch) => {
  db.collection("teachers")
    ...
    .catch((error) => {
      console.log(
        "Error fetching a particular TEACHER from teachers collection:",
        error,
      );
    });
};
  ...
  db.collection("teachers")
    ...
    .then((snapshot) => {
      ...
      if (existingTeacher.length) {
        notifyErrorFxn(
          `This teacher already exists,consider changing the name(s)`,
        );
      } else {
        db.collection("teachers")
          .add({
          ...
          .then((doc) => {
            ...
            notifySuccessFxn(
              `new Teacher ${
                addObject.firstName + " " + addObject.lastName
              } added!`,
            );
            setTimeout(() => {
  ...
  db.collection("sections")
    ...
    .then((snapshot) => {
      ...
      if (existingSubject.length) {
        notifyErrorFxn(
          `This subject already exists,consider changing the subject name`,
        );
      } else {
        db.collection("sections")
          .add({
...
export const updateTeacher =
  (uid, updateObject, navigate) => async (dispatch) => {
    db.collection("teachers")
      .doc(uid.trim())
      .update({
        body: updateObject.body,
...
export const addChapter = (addObject) => async (dispatch) => {
  db.collection("chapters")
    ...
    .then((snapshot) => {
      ...
      if (existingSubject.length) {
        notifyErrorFxn(
          `This chapter already exists,consider changing the chapter name`,
        );
      } else {
        db.collection("chapters")
          .add({
...
export const addPastExam = (addObject) => async (dispatch) => {
  db.collection("pastExams")
    ...
    .then((snapshot) => {
      ...
      if (existingSubject.length) {
        notifyErrorFxn(
          `This exam already exists,consider changing the exam name`,
        );
      } else {
        db.collection("pastExams")
          .add({
    ...
    .then((doc) => {
      if (doc.exists) {
        ...
        db.collection("pastExams")
          ...
          .get()
          .then((snapshot) => {
            const pastExamsArray = snapshot.docs.map((doc) => ({
              ...doc.data(),
            }));
            if (pastExamsArray.length) {
              console.log(
                `pastExams for The subject ${sectionId} are:`,
                pastExamsArray,
              );
              dispatch(saveSubjectPastExams(pastExamsArray));
              return pastExamsArray;
  ...
  db.collection("boneCourses")
    ...
    .then((snapshot) => {
      ...
      if (existingLesson.length) {
        notifyErrorFxn(
          `This lesson already exists,consider changing the lesson name`,
        );
      } else {
        db.collection("boneCourses")
          .add({
  ...
  db.collection("quizzes")
    ...
    .then((snapshot) => {
      ...
      if (existingQuiz.length) {
        notifyErrorFxn(
          `This quiz already exists,consider changing the quiz name`,
        );
      } else {
        db.collection("quizzes")
          .add({
...
export const updateVideoAndUserWatchlists =
...
export const addSubjectToPack =
  (subjectId, packId, packSubjects) => async (dispatch) => {
    ...
    db.collection("packs")
      ...
      .catch((error) => {
        console.error(
          "Error adding this subject to the pack, please view--> : ",
          error,
        );
        notifyErrorFxn(
          "Error adding this subject to the pack, please try again. ",
        );
      });
  };
...
export const fetchSubjectsBasedOnStudent =
  (studentId, email, setReadyList, setStudentId) => async (dispatch) => {
    dispatch(clearSubjectsForAdding());
    let correctId;
    ...
    db.collection("users")
      ...
      .then((snapshot) => {
        ...
        if (allGroups.length > 0) {
          ...
          notifyInfoFxn(
            "Courses have been populated, please choose from the dropdown below",
          );
        } else {
          dispatch(clearSubjectsForAdding());
...
export const updatePurchasedCourses =
  (studentId, email, newPurchasedCourses, navigate) => async (dispatch) => {
      ...
      .then((snapshot) => {
        ...
        if (allGroups.length > 0) {
          ...
          db.collection("users")
            ...
            .then((doc) => {
              if (doc.exists) {
                let newPurchasedCourseIds = newPurchasedCourses.map(
                  (item) => item.id,
                );
                db.collection("users")
                  .doc(studentId)
                  .update({
                    ...
                    purchasedCourses: [
                      ...doc.data().purchasedCourses,
                      ...newPurchasedCourseIds,
                    ],
                  });
              } else {
                db.collection("users")
                  ...
                  .then((snapshot) => {
                    ...
                    console.log(
                      "THE FOUND STUDENT to update his/HER course---->",
                      allGroups[0],
                    );
                    db.collection("users")
                      .doc(correctId)
            ...
            .then((doc) => {
              if (doc.exists) {
                db.collection("users")
                  .doc(studentId)
                  .update({
                    purchasedCourses: firebase.firestore.FieldValue.arrayUnion(
                      ...newPurchasedCourses,
                    ),
                    // courses:[...allGroups[0].courses,...newPurchasedCourses]
                  });
