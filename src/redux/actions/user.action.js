...
export const initiateConnection =
  (type, user1, user2, usedConnection) => (dispatch) => {
    dispatch(fetchConnection(user1, user2, type, usedConnection));
  };

export const fetchConnection =
  (user1, user2, type, usedConnection) => async (dispatch) => {
        ...
        if (querySnapshot.empty) {
              ...
              if (type == 1) {
                ...
                userRef
                  ...
                  .catch((error) => {
                    var errorMessage = error.message;
                    console.log(
                      "Error updating used connection:",
                      errorMessage,
                    );
                  });
              }
          ...
          querySnapshot.forEach((doc) => {
            ...
            if (
              ...
              var errorMessage =
                "You cannot invite, untill you undo this user from skipped";
              console.log("Error Msg: ", errorMessage);
              dispatch(initiateFailed({ errorMessage }));
  ...
  unsubscribe.onSnapshot((querySnapshot) => {
    ...
    if (users.length > 0) {
      db.collection("users")
        ...
        .get()
        .then((snapshot) => {
          const connectedUsers = snapshot.docs.map((doc) => ({
            ...doc.data(),
          }));
...
export const updateConnection =
  (user1, user2, status, history) => async (dispatch) => {
    var connect = db.collection("connections");
    connect = connect.where("user1", "==", user1);
    connect = connect.where("user2", "==", user2);
    connect
...
export const fetchEmployeeContacts = (contacterId) => async (dispatch) => {
  ...
  try {
    ...
    const employeeContacts = allContacts.filter(
      (contact) => contact.contacterId === contacterId,
    );
